...
export async function middleware(request: NextRequest) {
    ...
    const isOrganizationSelectionRoute = pathname === '/org';
    ...
    if (isAuthenticated) {
        ...
        // Allow authenticated users to access organization selection page
        ...
        // If not authenticated, allow access to auth routes (login, signup, etc.)
        if (isAuthRoute) {
            return NextResponse.next();
        }
        
        // If not authenticated and trying to access organization selection, redirect to login
        if (isOrganizationSelectionRoute) {
            return NextResponse.redirect(new URL('/login?from=org', request.url));
        }
